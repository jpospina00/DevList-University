...
export default function HistoryRequestDevices() {
  ...
  return (
    <div className="h-full w-full flex flex-col items-center gap-10">
      <h1 className="font-montserrat font-semibold text-[24px] mt-4">
      ...
      <div className="flex items-center justify-around w-[90%]">
        ...
        <div className="w-[500px] relative">
          <input
            ref={search}
            className="border border-[#229799] w-full rounded-xl p-1 outline-none"
            type="text"
            placeholder="Buscar"
            onChange={handleSearch}
          />
          <img
            className="absolute top-3 right-3 w-[17px] h-[14px]"
            src={SearchIcon}
            alt="Buscar"
          />
        </div>
        <div className="flex items-center justify-center gap-5 mt-2">
          <p className="font-montserrat font-medium text-[16px] pt-1">
            Mostrar
          </p>
          <Dropdown
        ...
        <Link to={'/request-devices'} className="py-2 px-5 border border-secondary0 rounded-lg hover:bg-secondary0Hover hover:text-disable ">
          Volver
        </Link>
      </div>
        ...
        <table className="w-[95%] table-fixed text-center">
          ...
          <tbody className="font-montserrat font-normal text-[14px]">
            {list.map((index) => (
              <tr
                key={index}
                className="border-b-2 border-dark w-[90%] border-opacity-20"
              >
                <td className="flex items-center justify-center p-5">
                  <img
                    className="w-[70%] h-[70%] rounded-lg border border-dark"
                    src={Image}
                    alt="Image"
                  />
                </td>
                ...
                <td>
                  <button
                    onClick={returnDevices}
                    className="border rounded-lg py-2 px-5 hover:bg-secondary0Hover hover:text-disable"
                  >
                    Devolver
                  </button>
                </td>
